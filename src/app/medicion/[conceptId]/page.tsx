...
export default async function MedicionConceptPage({ params }: Props) {
  const { conceptId } = await params
  const concept = getConcept(conceptId)
  if (!concept || concept.moduleId !== 'medicion') notFound()
  ...
  return (
    <AppLayout>
      ...
      <AIChat conceptId={concept.id} moduleId="medicion" />
    </AppLayout>
  )
}
