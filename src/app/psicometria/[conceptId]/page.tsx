...
export default async function PsicometriaConceptPage({ params }: Props) {
  const { conceptId } = await params
  const concept = getConcept(conceptId)
  if (!concept || concept.moduleId !== 'psicometria') notFound()
  ...
  return (
    <AppLayout>
      ...
      <AIChat conceptId={concept.id} moduleId="psicometria" />
    </AppLayout>
  )
}
